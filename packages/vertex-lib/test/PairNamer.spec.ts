...
describe('PairNamer', () => {
  ...
  describe('#pairName', () => {
    ...
    it('concatenation', async () => {
      expect(await pairName('DAI Stable Coin', 'MKR Token', 'Vertex Liquidity Provider Share for ', '.')).to.eq(
        'Vertex Liquidity Provider Share for DAI Stable Coin:MKR Token.'
      )
    })
  })
