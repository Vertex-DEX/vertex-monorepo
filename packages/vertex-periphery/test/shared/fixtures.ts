...
import IVertexPair from '@uniswap/v2-core/build/IVertexPair.json'
...
import VertexRouter01 from '../../build/VertexRouter01.json'
import UniswapV2Migrator from '../../build/UniswapV2Migrator.json'
import VertexRouter02 from '../../build/VertexRouter02.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  const router02 = await deployContract(wallet, VertexRouter02, [factoryV2.address, WETH.address], overrides)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IVertexPair.abi), provider).connect(wallet)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IVertexPair.abi), provider).connect(wallet)
